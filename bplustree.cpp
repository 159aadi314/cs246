...
void BPTree::insert_internal(Node* w, Node* p, Node* curr, int value) {
  ...
  w->num_keys++;
  w->keys[w->num_keys - 1] = value;
  w->children[w->num_keys] = p;
  for(int i = w->num_keys - 1; i > 0; i--) {
  ...
  if(w->num_keys < 2*t + 2) {
    ...
    for(int i = t + 1; i < 2*t + 2; i++) {
      ...
      p1->keys[i - t - 1] = a;
      p1->children[i - t - 1] = b;
      p1->children[i - t - 1]->parent = p1;

    }
    p1->children[t + 1] = w->children[2*t + 2];
    p1->children[t+1]->parent = p1;
  ...
  if(root->num_keys == 0) {
    ...
    while(true) {
      if(curr->isDataNode == true) {
        ...
        //   cout << curr->keys[1] << " ";
        // }
        curr->keys[curr->num_keys - 1] = value;
        for(int i = curr->num_keys - 1; i > 0; i--) {
        ...
        if(value < curr->keys[0]) {
          ...
          // if(value == 800) cout << curr->num_keys << " ";
        }
        else {
